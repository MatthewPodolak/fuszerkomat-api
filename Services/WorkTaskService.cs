...
namespace fuszerkomat_api.Services
{
    public class WorkTaskService : IWorkTaskService
    {
        ...
        public async Task<Result> PublishAsync(PublishWorkTaskVM model, string userId, CancellationToken ct)
        {
            ...
            {
                throw new ValidationException(message: "Max 5 images allwoed.");
            }
        ...
        public async Task<Result<UserWorkTaskVMO>> GetWorkTaskForUserAsync(int id, string userId, CancellationToken ct)
        {
            ...
            if (workTask == null)
            {
                throw new NotFoundException(logData: new { id, userId });
            }
        ...
        public async Task<Result<CompanyWorkTaskVMO>> GetWorkTaskForCompanyAsync(int id, string userId, CancellationToken ct)
        {
            ...
            if (workTask == null)
            {
                throw new NotFoundException(logData: new { userId, id });
            }
        ...
        public async Task<Result<ApplyVMO>> ApplyForWorkTaskAsync(ApplyToWorkTaskVM model, string companyId, CancellationToken ct)
        {
            ...
            {
                throw new ConflictException(logData: new { companyId, model });
            }
            ...
            {
                throw new NotFoundException(logMessage: $"gRpc server conv not found. {rpcEx}");
            }
            catch (RpcException rpcEx)
            {
                throw new InternalException(logMessage: $"gRpc server failed. {rpcEx}");
            }
        ...
        public async Task<Result> ChangeApplicationStatusAsync(ChangeApplicationStatusVM model, string userId, CancellationToken ct)
        {
            ...
            if (workTask == null)
            {
                throw new NotFoundException(message: "WorkTask does not exists.", logData: new { model.WorkTaskId });
            }

            var targetApplication = workTask.Applications.FirstOrDefault(a => a.CompanyUserId == model.CompanyId);
            if (targetApplication == null)
            {
                throw new NotFoundException(message: "This company didnt applicated for that task.");
            }
            ...
            switch (model.Answer)
            {
                case AnswerAplication.Accept:
                    ...
                    {
                        throw new ConflictException(message: "Another company has already been accepted.");
                    }
        ...
        public async Task<Result> CompleteRealization(CompleteRealizationVM model, string userId, CancellationToken ct)
        {
            ...
            if (workTask == null)
            {
                throw new NotFoundException(message: "Work task does not exists", logData: new { model.WorkTaskId });
            }
            ...
            if (application == null)
            {
                throw new NotFoundException(message: "Company didnt applied.", logData: new { model.WorkTaskId });
            }
        ...
        public async Task<Result<List<CompanyTaskApplyVMO>>> GetCompanyAppliedTasksAsync(CompanyAppliedFilterVM filter, string id, CancellationToken ct)
        {
            ...
            return Result<List<CompanyTaskApplyVMO>>.Ok(data: vmo, pagination: pagination, traceId: _http.HttpContext?.TraceIdentifier ?? string.Empty);
        }
    }
}
